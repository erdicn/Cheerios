...
void Simulate(cheerio_t* cheerios, int nb_cheerios, double rho_liq,// double rho_air,
                double rho_cheerio, double surface_tension, //double g, 
                    double capilary_length, long int NT, double dt, long long int* warning_counter, bords_t bords);
...
int main(){
    ...
    // soit on le mets directement soit on le calcule nous meme
    ...
    Simulate(cheerios, nb_cheerios, rho_liq, //rho_air,  // on les utilise pas dans la foncion donc pour pas avoir des warning je les ai enleve pour linstant TODO si a la fin aussi on les utilise pas enlever totalement
                rho_cheerio, surface_tension, //g, 
                    capilary_lenght, NT, dt, &warning_counter, bords);
...
void Simulate(cheerio_t* cheerios, int nb_cheerios, double rho_liq,// double rho_air,
    ...
    // B = CalculBondNumber(rho_cheerio ,rho_liq, R, surface_tension, 9.81, warning_counter);
    ...
    printf("B = %lf\nangle de contact = %lf degree\n", B, (theta*180)/M_PI);
    // printf("B = %lf \nB_lin = %lf\npi/2 * B = %lf\n%lf\n",CalculBondNumber(rho_cheerio ,rho_liq, R, surface_tension, 9.81, warning_counter),B, M_PI_2 * B ,theta);
    // printf("L_theta = %lf\ntheta = %lf\nT_theta = %lf\n", asin(CalculBondNumber(rho_cheerio ,rho_liq, R, surface_tension, 9.81, warning_counter)), asin(CalculLinearBondNumber(R,capilary_length, warning_counter)), acos((cheerios[0].m * 9.81)/(2)));
    ...
    double force = 0;
    double epsilon = capilary_length;
        ...
        for(c_obj = 0; c_obj < nb_cheerios; c_obj++){                                       // on parcour chaque cheerio pour calculer les forces qui sont excerce sur lui
            ...
            for(c = 0; c < nb_cheerios; c++){                                               // on calcul tout les forces par rapport tous les autres cheerios
                if(c != c_obj){                                                             // il exerce pas de force sur lui meme car sinon il essaye de prendre la distance entre lui meme et ca fait tel que l = 0 et l/L_c = 0 => et K1 ne marhe pas a 0
                    l = DistanceEntreDeuxCentreDeCheerios(cheerios[c_obj], cheerios[c]);            // prints pour voir si ca marche bien// printf("longueur entre %d et %d = %g m = %g cm\n", c_obj, c, l ,l*100);//printf("l = %.16lf L_c = %lf\n", l, capilary_length);//printf("l = %lf L_c = %lf l/L_c = %lf\n", l, capilary_length, l/capilary_length);
                    // TODO si on a l > epsilon de faireque il ya un pui pour que les cheerios se cogne
                    if( l > 0 + epsilon ){                                                              // car si cest 0 notre K1 est 0 et il est pas defini en 0 
                    ...
                    // } else if (l < epsilon && l > 0){
                    //     force = ForceBetweenTwoInteractingParticles(surface_tension, R, B, Sigma, l, capilary_length)/10.;
                    //     sensDeLaForce = VectorTimesScalar(SensEntreC1aC2(cheerios[c], cheerios[c_obj]), -1);           // TODO faire tel que a partir de langle et la curvature dde lobjet ca fait tel que ils sais si la force cest repulsive ou atractive
                    //     force_avec_direction = VecteurAdition(force_avec_direction,  VectorTimesScalar(sensDeLaForce, force));
                    } else {
                        // force = ForceBetweenTwoInteractingParticles(surface_tension, R, B, Sigma, l/capilary_length, capilary_length)/1000.;
                        // sensDeLaForce = VectorTimesScalar(SensEntreC1aC2(cheerios[c], cheerios[c_obj]), -1);           // TODO faire tel que a partir de langle et la curvature dde lobjet ca fait tel que ils sais si la force cest repulsive ou atractive
                        // force_avec_direction = VecteurAdition(force_avec_direction,  VectorTimesScalar(sensDeLaForce, force));
                        if (WARNING_MESAGES){
